...
class ScreenTransTime2(unittest.TestCase):
    
    def test_K02_MainToAutoTable(self):
        print('*'*25)
        print('K2_MainToAutoTable Screen Transistion \n')
        ...
        #res1=self.snip_n_grab() #To count the first transistion
        time.sleep(Screendelay)
        #self.click(174,126)  # Click Operation on PIC6
        #print("Clicked Current Alarm Table Button \n")
        #time.sleep(Screendelay)
        #res2=self.snip_n_grab()
        time.sleep(Screendelay)
        #res=res1+res2
        res=res1
        if res!=0:
...
#     suite = unittest.TestLoader().loadTestsFromTestCase(ScreenTransTime2)
